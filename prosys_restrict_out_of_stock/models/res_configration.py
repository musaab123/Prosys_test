...
class SaleConfiguration(models.TransientModel):
    ...
    restrict_sale_user_ids = fields.Many2many('res.users', string='Available User')
...
class SaleOrder(models.Model):
    ...
    def check_margin(self):
        for order in self:
            restrect_sale = self.env['ir.config_parameter'].sudo().get_param('prosys_restrict_out_of_stock.restrect_sale', default=False)
            restrict_sale_user_ids = self.env['ir.config_parameter'].sudo().get_param('prosys_restrict_out_of_stock.restrict_sale_user_ids', default='')
            ...
            for line in order.order_line:
                ...
                qty_available = product_template.qty_available
                product_name = product_template.name
                product_qty = line.product_uom_qty

                # Scenario 1: Unavailable quantity with no restrictions
                if product_template.unavailable_qty:
                    continue  # Skip this line as it's allowed regardless of other conditions

                # Scenario 2: Restrict sale with authorized users
                if restrect_sale and current_user_id in restrict_sale_user_ids:
                    continue  # Skip this line as it's allowed for authorized users

                # Scenario 3: All other cases (restrict sale)
                if product_qty > qty_available:
                    raise ValidationError(_("You can't confirm this order because of the following reasons:\n"
                        ...
                        "To confirm this order, please contact the administrator.").format(product_qty, product_name, qty_available))
    ...
    #         for line in order.order_line:
    #             if line.product_template_id.unavailable_qty:
    #                 if restrect_sale:
    #                     if order.user_id.id not in restrict_sale_user_ids:
