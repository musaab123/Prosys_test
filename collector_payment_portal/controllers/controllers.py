    ...
    def _prepare_home_portal_values(self, counters):
        ...
        if 'payment_count' in counters:
            ...
            payment_count = request.env['account.payment'].sudo().search_count(domain) \
                if request.env['account.payment'].sudo().check_access_rights('read', raise_exception=False) else 0
...
# © 2020 Kanak Infosystems LLP. (<https://www.kanakinfosystems.com>).
    ...
    #     value = []
    #     type_id_domain = []
    #     collector_id = request.env['res.users'].sudo().search(type_id_domain)
    ...
    #         'collecter_id': request.env.user.id,
    #         'date': post.get('date'),
    ...
    #     journal_entry_vals_debit = {
    #         'move_id': move_id.id,
    #         'partner_id': payment.partner_id.id,
           
    #     }

    #     journal_entry_vals_credit = {
    #         'move_id': move_id.id,
    #         'partner_id': payment.partner_id.id,
           
    #     }

    #     request.env['account.move.line'].sudo().create(journal_entry_vals_debit)
    #     request.env['account.move.line'].sudo().create(journal_entry_vals_credit)
...
# Add any other methods or routes as needed in your Odoo module
