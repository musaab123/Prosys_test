...
class PortalAttendanceKnk(CustomerPortal):
    ...
    @http.route(['/portal/create_invoice'], type='json', auth="user", website=True)
    ...
    @http.route(['/portal/create_payment'], type='json', auth="user", website=True)
    def create_payment(self, **kw):
        print("KW ---------------------->",kw)

        kw['partner_id']= int(kw['partner_id'])
        kw['amount']= float(kw['amount'])

        print("KW ---------------------->",kw)
        order = request.env['account.payment'].sudo().create(kw)
        print("order ---------------------->", order)
    ...
    @http.route('/my/about/page', website=True ,page=True,auth='public')
    def maintenance_request_thanks(self):
        return request.render("prosys_portal_driver.customers_request_thank_page")
    
    @http.route('/my/payment/successfuly', website=True ,page=True,auth='public')
    def payment_request_thanks(self):
        return request.render("prosys_portal_driver.customers_request_payment_page")
