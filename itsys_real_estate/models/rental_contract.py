...
class rental_contract(models.Model):
    ...
    apply_tax= fields.Boolean('Tax exempt')
    #Building Info
    ...
    account_security_deposit= fields.Many2one('account.account', 'Insurance deposit account', default=_default_security_deposit_account)
    ...
    @api.onchange('date_from','date_to','rental_fee','insurance_fee','periodicity',
                  'recurring_interval','account_income','account_security_deposit')
        ...
        for rec in self:
            if rec.periodicity and rec.date_from and rec.date_to:
                ...
                if self.insurance_fee:
                    rental_lines.append(
                        (0, 0, {'serial': i, 'amount': self.insurance_fee, 'line_account': self.account_security_deposit.id,
                                'date': date_from, 'name': _('Insurance Deposit')}))
                    i += 1
                rental_lines.append((0, 0, {'serial': i, 'amount': rental_fee, 'line_account': self.account_income.id,
                                            'date': date_from, 'name': _('Rental Fee')}))
                i+=1
                ...
                while new_date < (date_to - relativedelta(**{periodicity:self.recurring_interval})):
                    ...
                    rental_lines.append((0, 0, {'serial': i, 'amount': rental_fee, 'date': new_date,
                                                'line_account': self.account_income.id,
                                                'name': _('Rental Fee')}))
                    i+=1
...
class loan_line_rs_rent(models.Model):
    ...
    line_account = fields.Many2one('account.account','Account',)
