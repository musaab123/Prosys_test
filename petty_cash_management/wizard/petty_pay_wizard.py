...
class PettyPayWizard(models.TransientModel):
    ...
    def _compute_hide_payment_method(self):
        ...
        # Check if the attribute 'outbound_payment_method_ids' exists on the journal
        if hasattr(self.journal_id, 'outbound_payment_method_ids'):
            journal_payment_methods = self.journal_id.outbound_payment_method_ids
            self.hide_payment_method = (
                len(journal_payment_methods) == 1
                and journal_payment_methods[0].code == 'manual'
            )
        else:
            # If the attribute is not present, handle it accordingly
            # You might want to log a warning or handle this case based on your requirements
