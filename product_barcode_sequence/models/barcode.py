...
class ProductTemplate(models.Model):
    ...
    def _check_unique_barcode(self):
        for product in self:
            code = ""
            if product.active and (product.barcode):
                domain = [('active', '=', True)]
                if product.barcode:
                    domain.extend([
                        '|',
                ...
                if product.barcode:
                    domain.append(('barcode', '=', product.barcode))
                ...
                if duplicate_products:
                    if product.brand_id:
                        code += str(product.brand_id.code)
                        
                    if product.factory_id:
                        code += str(product.factory_id.code)

                    new_barcode = self.env['ir.sequence'].next_by_code('barcode.sequence')
                    print("ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc",new_barcode)
                    product.barcode = str(code) + str(new_barcode)
                    

                    product.barcode_check = True
                    product.original_code = new_barcode
                    ...
                    if product.brand_id:
                        code += str(product.brand_id.code)
                    # if product.hscode_id:
                    #     code += str(product.hscode_id.code)
                    # if product.categ_id:
                    #     code += str(product.categ_id.code)
                    
                    product.barcode = str(code) + str(product.original_code)
                    product.original_barcode = product.barcode
                for varient in product.product_variant_ids:
                    if varient.product_tmpl_id:
                        code = ""
                        # if varient.product_tmpl_id.factory_id:
                        #     code += str(varient.product_tmpl_id.factory_id.code)
                        ...
                        varient.barcode = code
                        # varient.default_code = temp_id_barcode
                        varient.product_tmpl_id.barcode = product.original_barcode
        ...
        for template in self:
            ...
            for varient in template.product_variant_ids:
                if varient.product_tmpl_id:
                    ...
                    # varient.barcode = code
                    varient.default_code = temp_barcode

                    # varient.default_code = code
                    # varient.product_tmpl_id.barcode = temp_barcode
                    # varient.product_tmpl_id.default_code = temp_barcode
