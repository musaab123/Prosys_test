...
class SetuScoreConfigurationCreator(models.TransientModel):
    ...
    def confirm(self):
        action = self.env.ref('prosys_customer_rating.setu_score_configuration_creator_action').read()[0]
        action['view_id'] = (self.env.ref('prosys_customer_rating.setu_score_configuration_creator_message_form_view').id,
                             'score_configuration_exists_form_warning')
        action['views'] = [
            (self.env.ref('prosys_customer_rating.setu_score_configuration_creator_message_form_view').id, 'form')]
