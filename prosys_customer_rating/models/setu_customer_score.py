...
class SetuCustomerScore(models.Model):
    ...
    def run_customer_score_cron(self):
        ...
        if companies_to_evaluate:
            ...
            days = int(
                self.env['ir.config_parameter'].sudo().get_param('prosys_customer_rating.customer_score_days')) or 365
            invoice_days = int(
                self.env['ir.config_parameter'].sudo().get_param('prosys_customer_rating.past_days_for_invoice')) or 60
            invoice_due_date_limit = date.today() - relativedelta(days=invoice_days)
            date_limit = str(date.today() - relativedelta(days=days))
            months = int(days / 30)
            if months == 0:
                months = 1
            score_conf_avg_sales_amt_id = self.env.ref('prosys_customer_rating.score_conf_avg_sales_amt').id
            score_conf_avg_monthly_sales_refund_id = self.env.ref(
                'prosys_customer_rating.score_conf_avg_monthly_sales_refund').id
            score_conf_qty_invoice_paid_after_due_id = self.env.ref(
                'prosys_customer_rating.score_conf_qty_invoice_paid_after_due').id
            score_conf_amt_invoice_paid_after_due_id = self.env.ref(
                'prosys_customer_rating.score_conf_amt_invoice_paid_after_due').id
            score_conf_qty_invoice_paid_after_x_days_id = self.env.ref(
                'prosys_customer_rating.score_conf_qty_invoice_paid_after_x_days').id
            score_conf_amt_invoices_due_after_x_days_id = self.env.ref(
                'prosys_customer_rating.score_conf_amt_invoices_due_after_x_days').id
            score_conf_avg_payment_days_id = self.env.ref('prosys_customer_rating.score_conf_avg_payment_days').id
