...
class OnBoardingController(http.Controller):
    ...
    @http.route('/prosys_customer_rating/evaluate', auth='user', type='http')
    def evaluate_customer_rating(self, **kwargs):
        ...
        action_id = request.env.ref("prosys_customer_rating.setu_customer_rating_kanban_action").id
        menu_id = request.env.ref('prosys_customer_rating.setu_customer_rating_dashboard_menu').id
    ...
    @http.route('/prosys_customer_rating/customer_rating_upper_panel', auth='user', type='json')
    def customer_rating_upper_panel(self):
        ...
        if rules:
            ...
            action_id = request.env.ref('prosys_customer_rating.setu_score_configuration_action').id
            menu_id = request.env.ref('prosys_customer_rating.setu_customer_rating_main_menu').id
        ...
        return {
            'html': request.env['ir.ui.view']._render_template(
                'prosys_customer_rating.customer_rating_upper_panel', {
                    'message': message,
                    'link': link,
                    'has_rules': has_rules,
                    'state': company.customer_rating_calculated
...
class CustomerRatingDashboard(http.Controller):
    ...
    @http.route('/prosys_customer_rating/fetch_rating_dashboard_data', type="json", auth='user')
    def fetch_dashboard_data(self, company_id):
        ...
        query = f"""
                    ...
                    inner join prosys_customer_rating cr on cr.id = substring(ip.value_reference,22)::integer 
