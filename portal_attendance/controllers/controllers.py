...
class PortalAttendanceKnk(CustomerPortal):
    ...
    def get_sh_attendance_manual(self, **kw):
        ...
        config_distance = float(request.env["ir.config_parameter"].sudo().get_param("sh_hr_attendance_geolocation_distance_id"))
        print("mmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmm",config_distance ,distance ,work_lat ,work_long ,latitude ,longitude)
        if not employee.is_representer: 
            if config_distance:
                if distance >= float(request.env["ir.config_parameter"].sudo().get_param("sh_hr_attendance_geolocation_distance_id")):


                    return {
                        'error': "Attendance not allowed. You are more than allowed meters away from the allowed location."
                    }

        vals = [kw.get('message'), latitude, longitude]
