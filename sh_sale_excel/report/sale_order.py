...
class SaleOrder(models.Model):
    ...
    def action_sale_order_xls(self):
        ...
        for sale_order in self.env['sale.order'].browse(active_ids):
            order_lines = []
            ...
            final_value['date_order'] = sale_order.date_order
            final_value['validity_date'] = sale_order.validity_date
            final_value['currency_id'] = sale_order.currency_id
            final_value['payment_term_id'] = sale_order.payment_term_id.name
            final_value['client_order_ref'] = sale_order.client_order_ref
            final_value['state'] = dict(self.env['sale.order'].fields_get(
            ...
            final_value['note'] = sale_order.note
            ...
            if sale_order.state not in ['draft', 'sent']:
                ...
                if final_value['validity_date']:
                    worksheet.write(11, 1, str(
                        final_value['validity_date']), format3)
            ...
            worksheet.write(5, 3, 'Customer Ref.', format1)
            if final_value['client_order_ref']:
                worksheet.write_merge(5, 5, 4, 5, str(
                    final_value['client_order_ref']), format3)
            else:
                worksheet.write_merge(
            ...
            worksheet.write(13, 10, "Taxes", bold)
            worksheet.write(13, 11, "Subtotal", bold)
            ...
            for rec in order_lines:
                ...
                if rec.get('tax_id'):
                    worksheet.write(row, 10, ",".join(
                        rec.get('tax_id')), format4)
                if rec.get('price_subtotal'):
                    worksheet.write(row, 11, rec.get('price_subtotal'), format4)
                # else:
                #     worksheet.write(row, 11, ",", format4)
            ...
            if final_value['note']:
                worksheet.write_merge(
                    row, row+1, 0, 5, final_value['note'], format3)
        ...
        return{
            ...
            'url': 'web/content/?model=excel.extended&field=excel_file&download=true&id=%s&filename=%s' % (export_id.id, export_id.file_name),
            'target': 'new',
        }
